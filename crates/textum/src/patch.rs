...
impl Patch {
    ...
    /// Apply this patch to a file, reading from and writing to disk.
    ///
    ...
    /// ```
    pub fn apply_to_file(&self) -> Result<(), PatchError> {
        ...
        std::fs::write(file_path, rope.to_string()).map_err(PatchError::IoError)?;
